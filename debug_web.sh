...
if [ "$#" -eq "0" ]; then
  ...
  echo "       make sure there is at least one port-map set for establish debug connection. Meanwhile, app-port in this port-map has to be same as the one exposed by specific app script, e.g. 'debug_web_app'" 
fi
